...
//--------------------------------------------------------------------------------------
    ...
    /*
      if (array_list_size(array_list_aux) > 0) {
	printf("####Report by anchor\n", read->id);
	array_list_clear(mapping_batch->mapping_lists[targets[i]], bwt_anchor_free);
	for (int a = array_list_size(array_list_aux) - 1; a >= 0; a--) {	  
	  alignment = array_list_remove_at(a, array_list_aux);
	  array_list_insert(alignment, mapping_batch->mapping_lists[targets[i]]);	
	}
      } 
      else {
	//For RNA	
	...
	//Anchor not valid. Seeding between anchors
	array_list_clear(mapping_batch->mapping_lists[targets[i]], bwt_anchor_free);
	num_mappings = bwt_map_exact_seeds_seq(0,
					       0,
					       read->sequence,
