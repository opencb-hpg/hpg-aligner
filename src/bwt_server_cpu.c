...
int apply_bwt(bwt_server_input_t* input, batch_t *batch) {
  ...
  } else if (batch->pair_input->pair_mng->pair_mode != SINGLE_END_MODE && batch->mapping_mode == DNA_MODE) {      
    return PRE_PAIR_STAGE;
  } else if (batch->mapping_mode == RNA_MODE && batch->pair_input->pair_mng->pair_mode != SINGLE_END_MODE) {
