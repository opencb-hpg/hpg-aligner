...
/*
	       if (found) {
		 array_list_insert(fastq_read_dup(fq_read), reads_new);
		 mb_new->targets[limit_of_reads] = 1;
		 limit_of_reads++;
		 if (limit_of_reads >= MAX_READS_SP) {
		   mb_new->num_targets = limit_of_reads;
		   mb_new->fq_batch = reads_new;
		   limit_of_reads = 0;
		   linked_list_insert(batch_new, linked_list);
		   if (++n_insert > 5) {
		     n_insert = 0;
		     pthread_cond_signal(&cond_sp);
		   }

		   reads_new = array_list_new(MAX_READS_SP, 1.25f, COLLECTION_MODE_ASYNCHRONIZED);
		   mb_new = mapping_batch_new_by_num(MAX_READS_SP, mapping_batch->pair_mng);

		   batch_new = batch_new(batch->bwt_input, batch->region_input, batch->cal_input, 
					 batch->pair_input, batch->preprocess_rna, batch->sw_input, batch->writer_input, 
...
void write_to_bam_file(array_list_t *array_list, bam_file_t *bam_file) {
  size_t num_items = array_list_size(array_list);
  alignment_t *alig;
  bam1_t *bam1;
...
void write_read_no_mapped(fastq_read_t *fq_read, bam_file_t *bam_file) {
...
int bam_writer(void *data) {
     ...
     */
	 ...
	 if (!num_items) {
	   total_mappings++;
	   write_read_no_mapped(fq_read, bam_file);
	 } else {
	   ...
	   if (batch->pair_input->pair_mng->pair_mode == SINGLE_END_MODE) {
	     if (search_hard_clipping(mapping_batch->mapping_lists[i])) {
	       ...
	       num_mapped_reads++;
	       write_to_bam_file(mapping_batch->mapping_lists[i], bam_file);
	     }
	   } else {
	      if (i%2 == 0) {
		...
		if (found_p1 || found_p2) {
		  ...
		  write_to_bam_file(mapping_batch->mapping_lists[i], bam_file);
		  write_to_bam_file(mapping_batch->mapping_lists[i + 1], bam_file);
		}
		i++;
	      }
	   }
...
int pre_pair_stage(void *data) {
     batch_t *batch = (batch_t *) data;
     apply_pair(batch->pair_input, batch);
